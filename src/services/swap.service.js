...
class SwapService {
  constructor() {
    if (!SwapService.instance) {
      ...
      // we can check whether or not we’ve already instantiated a UserStore,
      // and if we have, we won’t create a new one.
  ...
  async findDeployedContractAddress() {
    ...
    try {
      ...
      console.debug(
        `Fetching user ${
          this.account
        } transactions from block ${from} to block ${latest}`
      );
  ...
  async getBalances() {
    const baseTokenBalance = await this.baseTokenContract.balanceOf(
      this.address
    );
  ...
  async deploy(baseToken, baseExchange) {
    ...
    try {
      this.contractInstance = await this.factory.deploy(
        baseToken,
        baseExchange
      );
    } catch (e) {
  ...
  async withdraw(amount, currency) {
    ...
    switch (currency) {
      ...
      default:
        throw new Error(
          `Currency ${currency} is not accepted by this contract`
        );
    }
  }
}
